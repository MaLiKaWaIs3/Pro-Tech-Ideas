...
import PrescriptionTable from '../MUITable/Table';
...
function StudentHistory() {
  ...
  const getPatientById = async () => {
    ...
    const response = await axios.get(`http://localhost:3002/patients/${id}`);
  ...
  const getHistory = async () =>{
    let response = await axios.get(`http://localhost:3002/patients/history/${patientId}`, 
        {
            headers: {
